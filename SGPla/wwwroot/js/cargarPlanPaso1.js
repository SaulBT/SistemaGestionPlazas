...
function inicializarControles() {
    ...
    configurarCambioRegion(
        region,
        area);

    configurarCambioArea(
        region,
        area,
        entidad);
...
async function configurarCambioRegion(
    region,
    area) {
    if (!region) {
        return;
...
async function configurarCambioArea(
    region,
    area,
    entidad) {
    if (!area) {
...
async function configurarCambioEntidad(
    region,
    area,
    entidad,
    programa) {
...
async function cargarOpcionesAsync(
    url,
    selectDestino,
    placeholder) {
    const response =
...
function actualizarVisibilidad() {
    const region =
        document.getElementById("RegionSeleccionada");

    const area =
        document.getElementById("AreaSeleccionada");

    const entidad =
        document.getElementById("EntidadSeleccionada");
