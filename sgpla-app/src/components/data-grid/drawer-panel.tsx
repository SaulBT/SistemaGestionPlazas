...
export function DrawerPanel<Row, ColumnId extends string>({
  ...
  return (
    <DialogPrimitive.Portal>
      <DialogPrimitive.Popup className="data-open:translate-x-0 data-closed:translate-x-full fixed top-0 right-0 z-50 flex h-dvh w-full max-w-md translate-x-full flex-col border-l-border bg-background shadow-xl ring-1 ring-foreground/10 outline-none transition-transform duration-200">
        <div className="flex items-center justify-between gap-3 border-b-border px-4 py-3">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          ...
          <dl className="grid gap-3 text-sm">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <dd className="text-right font-medium">
                {drawerColumn && isEditableColumn(drawerColumn.id)
                  ? "Yes"
                  : "No"}
              </dd>
            </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t-border p-3">
          <DialogPrimitive.Close
