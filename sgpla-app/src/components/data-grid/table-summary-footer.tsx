...
>({
  ...
  const stickyCellClassName = stickySummaryFooter
    ? "sticky bottom-0 z-10 border-t-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/90"
    : undefined

  return (
    <TableFooter className="border-b-border bg-background font-normal">
      <TableRow className="hover:bg-transparent">
        <TableCell className={cn("h-10 border-r-border px-2 py-1", stickyCellClassName)} />
        {visibleColumns.map((column) => {
          ...
          return (
            <TableCell
              ...
              className={cn(
                "h-10 border-r-border px-2 py-1 text-left text-xs whitespace-nowrap text-muted-foreground",
                stickyCellClassName,
